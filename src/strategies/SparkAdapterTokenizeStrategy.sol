...
/**
 * NapFi Hyper-Optimized Spark Tokenized Strategy v4.0 - "Spark Titan Vault"
 * -------------------------------------------------------------------------
 * ERC4626 compliant tokenized strategy integrating with SparkAdapterTokenizedAdapter
 * Full compatibility with Yearn V3/Octant vaults and BaseStrategy patterns
 */
...
contract SparkAdapterTokenizeStrategy is BaseHealthCheck {
    ...
    // Core Configuration
    // --------------------------------------------------
    NapFiHyperSparkAdapter public immutable hyperAdapter;
    address public immutable sparkMarket;
    ...
    constructor(
        ...
        require(_sparkMarket != address(0), "Invalid Spark market");
        
        hyperAdapter = NapFiHyperSparkAdapter(_hyperAdapter);
        sparkMarket = _sparkMarket;
        
        // Register strategy with adapter (following SparkAdapter pattern)
    ...
    /**
     * @dev Deploy funds to Spark via hyper-optimized tokenized adapter
    ...
    function _deployFunds(uint256 _amount) internal override {
        ...
        try hyperAdapter.supplyToSpark(sparkMarket, _amount) returns (uint256 shares) {
    ...
    /**
     * @dev Free funds from Spark via hyper-optimized tokenized adapter
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (safeWithdrawAmount > 0) {
            ...
            try hyperAdapter.withdrawFromSpark(sparkMarket, safeWithdrawAmount, address(this)) returns (uint256 withdrawn) {
                uint256 gasUsed = gasBefore - gasleft();
     ...
     * Following SparkAdapter registration pattern
     */
    ...
    function _executeHyperHarvest() internal {
        try hyperAdapter.harvestSparkRewards() returns (uint256 yield, uint256 donation) {
    ...
    function _executeOpportunisticRebalance() internal {
        ...
        if (bestMarket != sparkMarket && bestMarket != address(0)) {
            uint256 rebalanceAmount = _calculateOptimalRebalanceAmount();
            
            try hyperAdapter.rebalanceSparkMarkets() {
                lastRebalanceTimestamp = block.timestamp;
                
                uint256 estimatedGain = _estimateRebalanceGain(sparkMarket, bestMarket, rebalanceAmount);
                emit StrategyRebalanced(sparkMarket, bestMarket, rebalanceAmount, estimatedGain);
            } catch {
    ...
    function getCurrentAPY() public view returns (uint256) {
        try hyperAdapter.getSparkAPY(sparkMarket) returns (uint256 baseAPY) {
    ...
    function _getAdapterPositionValue() internal view returns (uint256) {
        ...
        try hyperAdapter.getStrategyPositionValue(address(this), sparkMarket) returns (uint256 value) {
            return value;
        } catch {
            return 0;
        }
    ...
    function _findBestPerformingMarket() internal view returns (address) {
        ...
        try hyperAdapter.findBestMarket(address(asset)) returns (address bestMarket) {
            ...
            return sparkMarket; // Fallback to current market
        }
    }
    ...
    function _executeEmergencyRecovery(uint256 _amount) internal {
        ...
        try hyperAdapter.emergencyWithdraw(_amount, address(this)) {
            ...
            try hyperAdapter.withdrawFromSpark(sparkMarket, _amount, address(this)) {
                // Fallback success
            } catch {
...
interface NapFiHyperSparkAdapter {
    function registerStrategy(address strategy, address asset) external;
    function supplyToSpark(address market, uint256 amount) external returns (uint256);
    function withdrawFromSpark(address market, uint256 amount, address to) external returns (uint256);
    function harvestSparkRewards() external returns (uint256 yield, uint256 donation);
    ...
    function getSparkAPY(address market) external view returns (uint256);
    function shouldRebalance(address strategy) external view returns (bool);
    function rebalanceSparkMarkets() external;
